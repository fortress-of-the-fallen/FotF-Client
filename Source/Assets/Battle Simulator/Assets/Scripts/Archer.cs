...
public class Archer : MonoBehaviour {
	...
	void LateUpdate(){
		...
		if(addArrowForce && this.gameObject != null && GetComponent<UnitV2>().currentTarget != null && newArrow != null && arrowSpawner != null){
			//create a shootingforce
			shootingForce = Vector3.Distance(transform.position, GetComponent<UnitV2>().currentTarget.transform.position);
			...
			((GetComponent<UnitV2>().currentTarget.transform.position.y - transform.position.y) * 45), shootingForce * 55)));
			addArrowForce = false;
		}
	}
